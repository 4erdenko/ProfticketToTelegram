...
@analytics_router.message(F.text == LEXICON_BUTTONS_RU['/back_to_analytics_menu'])
...
async def cmd_back_to_main_menu(message: Message, session: AsyncSession, state: FSMContext):
    await state.clear()
    await message.answer(
...
@analytics_router.message(F.text.in_([
    LEXICON_BUTTONS_RU['/report_top_shows_sales'],
    LEXICON_BUTTONS_RU['/report_top_shows_speed'],
    LEXICON_BUTTONS_RU['/report_top_artists_sales']
]))
async def cmd_select_report_type(message: Message, state: FSMContext, session: AsyncSession):
    # Получаем все доступные месяцы из базы
    shows = (await session.execute(select(Show))).scalars().all()
    months = sorted(set((s.month, s.year) for s in shows if s.month and s.year))
    if not months:
...
@analytics_router.message(F.text == LEXICON_BUTTONS_RU['/report_predict_sell_out'])
async def cmd_select_soldout_report(message: Message, state: FSMContext, session: AsyncSession):
    # Получаем все доступные месяцы из базы
    shows = (await session.execute(select(Show))).scalars().all()
    months = sorted(set((s.month, s.year) for s in shows if s.month and s.year))
    if not months:
...
async def cmd_generate_top_report_month(message: Message, session: AsyncSession, state: FSMContext):
    ...
    await message.answer(LEXICON_RU['WAIT_MSG'], reply_markup=analytics_main_menu_keyboard())
    all_shows = (await session.execute(select(Show))).scalars().all()
    all_histories = (await session.execute(select(ShowSeatHistory))).scalars().all()
    if not all_shows or not all_histories:
        await message.answer(LEXICON_RU['NO_DATA_FOR_REPORT'])
        return
    results = analytics_func(shows=all_shows, histories=all_histories, month=month, year=year, n=10)
    if not results:
    ...
    if report_type_key == LEXICON_BUTTONS_RU['/report_top_shows_sales']:
        ...
        for i, (artist, sold) in enumerate(results, 1):
            response_lines.append(f"{i}. {artist} — уч. в продажах: {sold} бил.")
        ...
        for i, (name, rate_sec, _id) in enumerate(results, 1):
            ...
            response_lines.append(f"{i}. {name} — ~{rate_day:.1f} {LEXICON_RU['SALES_SPEED_UNIT_PER_DAY']}")
...
async def cmd_generate_soldout_report(message: Message, session: AsyncSession, state: FSMContext):
    user_data = await state.get_data()
    report_type_key = user_data.get('report_type_to_generate')
    await state.clear()
    try:
        ...
        await message.answer(LEXICON_RU['ERROR_MSG'])
        return
    analytics_func = REPORTS[LEXICON_BUTTONS_RU['/report_predict_sell_out']]['handler']
    report_title = REPORTS[LEXICON_BUTTONS_RU['/report_predict_sell_out']]['title']
    period_text = f" (за {text})"
