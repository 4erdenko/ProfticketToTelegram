...
def analytics_main_menu_keyboard() -> ReplyKeyboardMarkup:
    ...
    builder = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [
                KeyboardButton(text=LEXICON_BUTTONS_RU['/report_top_shows_sales']), 
                KeyboardButton(text=LEXICON_BUTTONS_RU['/report_top_shows_speed'])
            ],
            # Объединяем кнопки прогноза и артистов в один ряд  
            [
                KeyboardButton(text=LEXICON_BUTTONS_RU['/report_predict_sell_out']), 
                KeyboardButton(text=LEXICON_BUTTONS_RU['/report_top_artists_sales'])
            ],
            # Кнопка навигации
...
def analytics_period_choice_keyboard(report_command_base: str) -> ReplyKeyboardMarkup:
    """
    ...
    builder = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [
                ...
                KeyboardButton(text=LEXICON_BUTTONS_RU['/period_current_month']),
            ],
            # Кнопка навигации
...
def analytics_months_keyboard(months: list[tuple[int, int]]) -> ReplyKeyboardMarkup:
    """
    ...
    keyboard.append([KeyboardButton(text=LEXICON_BUTTONS_RU['/back_to_analytics_menu'])])
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True, is_persistent=False)

def analytics_months_with_alltime_keyboard(months: list[tuple[int, int]]) -> ReplyKeyboardMarkup:
    now = datetime.now(DEFAULT_TIMEZONE)
    filtered = [(m, y) for m, y in months if (y, m) >= (now.year, now.month)]
